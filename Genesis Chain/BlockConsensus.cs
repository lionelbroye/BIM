...
namespace firstchain
{
    public partial class Program
    {
        ...
        public static byte[] ApplyTheSeaToTheCryptoPuzzle(byte[] Current_Target, float currentwaterlevel)
        {
           
            float refpoint = SEA_MAXLEVEL;
            if (currentwaterlevel > SEA_MAXLEVEL) { currentwaterlevel = SEA_MAXLEVEL; }
            //Console.WriteLine("RESULT AT HEIGHT " + currentwaterlevel);
            float prct = currentwaterlevel / refpoint;
            
            float POWEROFTHEATLANTIC = SEA_FORCE - prct * SEA_FORCE;
           // Console.WriteLine("atlantic power : " + POWEROFTHEATLANTIC);
            // then scale it hard between 1 and sea force
            if (POWEROFTHEATLANTIC < 1) { POWEROFTHEATLANTIC = 1; } // raw stuff. kinda bad.


            prct = (float)Math.Pow(prct, POWEROFTHEATLANTIC); // should be always 2 .... 
            ulong PRECISION = SEA_UINT256_PRECISION; // using this current precision could be more 0. will see. like 1000 is like 0.001 but we need more like 1000000
            //prct *= PRECISION;
            decimal prctd = (decimal)(prct); // should do something if precision is higher value than max float precision
            prctd *= PRECISION;
            ulong prctInt = (ulong)prctd;
            ...
            b1 = BigInteger.Multiply(b1, mult); // mult is equal to 0 .... 
            ...
            byte[] result = Uint256ToByteArray(b1);
            Console.WriteLine("--------------------------------------------------------");
            Console.WriteLine("Tidal Prev Height At St.Nazaire            : " + currentwaterlevel);
            Console.WriteLine("Current Hash Target without sea consensus  : " + SHAToHex(Current_Target,false));
            Console.WriteLine("Sea modifier                               : " + prct);
            Console.WriteLine("Hash Target from the sea                   : " + SHAToHex(result, false));
            Console.WriteLine("--------------------------------------------------------");
