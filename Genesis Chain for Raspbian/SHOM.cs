...
namespace firstchain
{
    public class SHOM
    {
        ...
        public static List<SHOMData> GetShomDatafromPeriod(int station, uint tsStart, uint tsEnd)
        {
            ...
            DateTime dateBegin= UnixTimeStampToDateTime(tsStart);
            dateBegin = dateBegin.AddDays(-1);//dateStart.AddMinutes(-15);
            DateTime dateStop = UnixTimeStampToDateTime(tsEnd);
            dateStop = dateStop.AddDays(1); // we add one only if dateEnd is today! 
            Console.WriteLine("jour demandé = " + dateStop.Day.ToString());

            TimeSpan nbdays = dateStop.Subtract(dateBegin);
            int nTimesRequest = 1;
            DateTime dateStart = dateBegin;
            DateTime dateEnd = dateStop;
            if ( nbdays.Days > 31)
            {
                Console.WriteLine("is supp");
                dateEnd = dateBegin.AddMonths(1);
                nTimesRequest = (int)Math.Ceiling(nbdays.Days / 31f);
            }
            Console.WriteLine("number of demand = " + nTimesRequest);
            List<SHOMData> shoms = new List<SHOMData>();
            for ( int i = 0; i < nTimesRequest; i++)
            {
                ...
                Console.WriteLine(datestartstr);
                ...
                foreach( SHOMData shom in GetAllSHOMData(dtreceived.ToCharArray()))
                {
                    shoms.Add(shom);
                }
                dateStart = dateEnd;
        ...
        // Proccess JSON to SHOMData Function
        ...
        public static SHOMData StringToSHOMData(string result)
        {
            ...
            string fparsing = parser[2];//parser[2].Replace('.', ',');
        ...
        // Request Specific SHOMData Function
