...
namespace firstchain
{
    public class network
    {
        ...
        public class Server
        {
            ...
            public void StartListener()
            {
                try
                {
                    while (true)
                    {
                        Program.Print("Waiting for connections...");
                        TcpClient client = server.AcceptTcpClient();
                        Program.Print("New client connected!");
            ...
            public void HandleDeivce(Object obj)
            {
                ...
                string current_packet_identifier = "0";
                ...
                try
                {
                    while ((i = stream.Read(bytes, 0, bytes.Length)) != 0) //< ca process seulement lorsque je recoit qqchose
                    {
                        ...
                        {
                            if ( !Program.PendingDLBlocks.Contains(current_packet_identifier) && !Program.PendingDLTXs.Contains(current_packet_identifier))
                            {
                                Program.dlRemovalQueue.Add(current_packet_identifier); // should add it if it is not in pending dl block or pending dl tx && if not in
                            }
                            ...
                            for (int n = 9; n < 41; n++)
                            {
                                checksumbytes[n - 9] = bytes[n];
                            }
                            string checksumstring = Program.SHAToHex(checksumbytes, false);
                            if ( checksumstring == null) { Program.Print("checksum NULLLL !!!!!!!!!!!!!!"); }
                            current_packet_identifier = checksumstring;
                            byteOffset = 0;
                            pNumb = 0;
                            if (file_length < BUFFER_CHUNK - 41) // REMINDER : 41 bytes is the length of the header of the first packet
                            {
                                ...
                                File.WriteAllBytes(Program._folderPath + "net/" + current_packet_identifier + "_" + pNumb.ToString(), bwrite);
                                Program.Print("file downloaded! --->  " + current_packet_identifier);
                                switch (data_flag)
                                {
                                    case 1:
                                        Program.PendingDLBlocks.Add(current_packet_identifier);
                                        break;
                                    case 2:
                                        Program.PendingDLTXs.Add(current_packet_identifier);
                                        break;
                                }
                            ...
                            else
                            {
                                ...
                                Program.Print("downloading status : " + byteOffset + "/" + file_length); // OK ! 
                            }
                        ...
                        else
                        {
                            ...
                            if (file_length < byteOffset + BUFFER_CHUNK - 4) // Reminder : header 4 bytes for other packet
                            {
                                ...
                                Program.Print("downloading status :" + byteOffset + "/" + file_length + " " + bwrite.Length); // OK !
                                Program.Print("file downloaded!--->  " + current_packet_identifier);
                                switch (data_flag)
                                {
                                    case 1:
                                        if (!Program.PendingDLBlocks.Contains(current_packet_identifier))
                                        {
                                            Program.PendingDLBlocks.Add(current_packet_identifier);
                                        }
                                        ...
                                        break;
                                    case 2:
                                        if (!Program.PendingDLTXs.Contains(current_packet_identifier))
                                        {
                ...
                catch (Exception e)
                {
                    if (!client.Connected)
                    {
                        Program.Print("peer disconnect");
                    }
                    else
                    {
                        Program.Print(e.ToString());
                    }
                    Program.Print("peer removed");
                    client.Close();
        ...
        public void Initialize()
        {
            ...
            {
                Program.Print("Please put net info !");
                return;
            }
        ...
        public void SendData(byte[] data, int peerIndex) //<  send data to a peer! 
        {
            ...
            catch (Exception e)
            {
                ...
                if (!mPeers[peerIndex].Peer.Connected)
                {
                    ...
                    Program.Print("peers removed due to inactivity!"); 
                    */
        ...
        public void SendFile(string _fPath, byte header, int i) // this is kind of awful
        {
            ...
            */
            ...
            while (byteOffset < fLength)
            {
                ...
                SendData(Program.ListToByteArray(DataBuilder), i);
                byteOffset += chunk;
                Program.Print("uploading status : " + byteOffset + "/" + fLength + " " + chunk);
            }
            mPeers[i].currentlySending = false;
        ...
        public Tuple<uint[], byte[]> GetChunkBytesinBlockchainFiles(uint chunksize, uint byteOffset, uint fileOffset) //< this will help ! :3
        {
            ...
            Program.Print("chinksize = " + chunksize);
            List<byte> DataBuilder = new List<byte>();
            ...
            while (DataBuilder.Count < chunksize)
            {
                ...
                Program.Print("retrieving data : " + DataBuilder.Count + "/" + chunksize + " " + chunk);
                if (chunk != chunksize)
                {
                    fileOffset++;
                    byteOffset = 4;
        ...
        public void SendBlockChain(uint indexStart, uint offsetStart, string fileStart, uint indexEnd, uint offsetEnd, string fileEnd, int i)
        {
            ...
            Program.Print(offsetStart + " " + offsetEnd);
            mPeers[i].currentlySending = true;
            ...
            else
            {
                ...
                Program.Print("uploading status : " + byteOffset + "/" + fLength);
            }
            while (byteOffset < fLength)
            {
                DataBuilder = new List<byte>();
        ...
        public void BroadcastBlockchain(uint start, uint end) //< WILL SEND A LIST OF BLOCKS FROM INDEX START TO INDEX END 
        {
            ...
            if (sInfo == null || eInfo == null) { Program.Print("unable to broadcast blockchain beacause of bad pointer"); return; }
            if (end < start) { return; }
            for (int i = 0; i < mPeers.Count; i++)
            {
                int index = i;
