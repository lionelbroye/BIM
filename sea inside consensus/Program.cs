...
namespace firstchain
{
    ...
    {
        /*
          we recompute the hash for each block from shomData
          [*] request : from timestamp a to timestamp b ( firsttempindex to lasttempindex ) ( split this every 31 day min 1 day ) 
          then for each block re-compute hashtarget from shomdata corresponding to timestamp of the block (last shomdata at this timestamp )
       ...
       public class BroadcastInfo
       {
            ...
            while ( true)
            {
                ...
                if ( argument.Contains("rqshom"))
                {
                    ...
                    if ( int.TryParse(s, out index) )
                    {
                        ...
                        SHOM.TestSHOMData(index, amodif, tomorrow);
                        argumentFound = true;
                    }
                    else
                    {
        ...
        public static void ProccessTempBlocks(string _filePath, bool needPropagate) // MAIN FUNCTION TO VALID BLOCK
        {
            ...
            List<SHOM.SHOMData> shoms = SHOM.GetShomDatafromPeriod(37, bfirst.TimeStamp, blast.TimeStamp);
            //-----------
            ...
            if (HardFork)
            {
                ...
                while (true)
                {
                    ...
                    if (shom == null) { Console.WriteLine("No SHOM found... "); File.Delete(_filePath);
                        return;
                    }
                    tempTarget = ApplyTheSeaToTheCryptoPuzzle(tempTarget, shom);
                    //----
        ...
        public static void MineBlock(List<Tx> TXS, Block prevBlock, byte[] pKey, uint mUTXOP)
        {
            ...
            // get the shom data ... 
            List<SHOM.SHOMData> shoms = SHOM.GetShomDatafromPeriod(37, unixTimestamp, unixTimestamp);
            SHOM.SHOMData shom = SHOM.GetLastDataBeforeSpecificTime(SHOM.UnixTimeStampToDateTime(unixTimestamp), shoms);
            HASH_TARGET = ApplyTheSeaToTheCryptoPuzzle(HASH_TARGET, shom);
