    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                {
                    if ( e is NullReferenceException || e is ArgumentOutOfRangeException || e is ArgumentNullException)
                    {
                            ...
                            if (Continue)
                            {
                                ...
                                if (nTime == 0)
                                {
                                    while (true)
                                    {
                                        ...
                                        string winblockPath = StartMining(pkeyHASH, utxopointer, mnlock, 1);
                                ...
                                else
                                {
                                    ...
                                    string winblockPath = StartMining(pkeyHASH, utxopointer, mnlock, nTime);
                                    if ( winblockPath.Length != 0)
                                    {
                                        PendingBlockFiles.Add(new Tuple<bool, string>(true,  winblockPath));
                                    }
        ...
        public static void ProccessTempBlocks(string _filePath, bool needPropagate) // MAIN FUNCTION TO VALID BLOCK
        {
            ...
            if (!isHeaderCorrectInBlockFile(_filePath)) { Print("header incorrect!"); File.Delete(_filePath); return; }
            ...
            {
                if (firstTempIndex == 0) { Print("NO GENESIS ALLOWED !"); File.Delete(_filePath); return; }
                if (latestTempIndex < latestOfficialIndex + WINNING_RUN_DISTANCE) { Print("NOT WINNING DIST"); File.Delete(_filePath); return; }
                HardFork = true;
            }
            else
            {
            ...
            if (HardFork)
            {
                ...
                for (uint i = firstTempIndex - 1; i >= 0; i--)
                {
                    if (i == uint.MaxValue) { break; }
                    Block b = GetBlockAtIndex(i);
                    if (b == null) { Print("[missing block]"); File.Delete(_filePath); return; }
            ...
            else
            {
                ...
                if (currentBlockReading == null) { File.Delete(_filePath); Print("wrong index specified 1"); return; }
                ...
                if (firstTempIndex != latestOfficialIndex + 1) // searching A fork : if firsttempindex(1) is not latestofficialindex  (0) + 1
                {
                    ...
                    if (latestIndex > firstTempIndex - 1)
                    {
                        ...
                        {
                            File.Delete(_filePath); Print("[1]Can't find a fork to process those blocks. temp index : " + firstTempIndex); return;
                        }
                    }
                    else
                    {
                        ...
                        if (forkpath.Length == 0) { Print("[2]Can't find a fork to process those blocks. temp index : " + firstTempIndex); File.Delete(_filePath); return; }
                        else { _pathToGetPreviousBlock = forkpath; }
                        Block bb = GetBlockAtIndexInFile(latestTempIndex, _filePath);
                        if (bb == null) { Print("[missing block]"); File.Delete(_filePath); return; }
                        if (isForkAlreadyExisting(bb)) { Print("File Already Existing"); File.Delete(_filePath); return; }
                    }
                }
                else
                {
                ...
                if (previousBlock == null) { File.Delete(_filePath); Print("wrong index specified 2"); return; }
        ...
        public static Tuple<bool, List<UTXO>> IsBlockValid(Block b, Block prevb, uint MIN_TIME_STAMP, byte[] HASHTARGET, List<UTXO> vUTXO)
        {
            ...
            if (!isNonceGolden(hash, b.HashTarget)) { Print("wrong nonce"); return new Tuple<bool, List<UTXO>>(false, vUTXO); }
        ...
        public static string StartMining(byte[] pKey, uint mUTXOP, uint MAXLOCKTIME, uint NTIMES)
        {
            ...
            double unixTimestampB = (uint)(DateTime.UtcNow.Subtract(new DateTime(2020, 1, 1))).TotalMilliseconds;
            byte[] checksum = Program.ComputeSHA256(BitConverter.GetBytes(unixTimestampB));
            string WinBlockPath = _folderPath + "net\\" + SHAToHex(checksum, false);
            ...
            for (uint a = 0; a < NTIMES; a++)
            {
                ...
                {
                    Tx TX = BytesToTx(GetBytesFromFile(byteOffset, 1100, _folderPath + "ptx"));
                    if (TX == null) { Print("err here"); return ""; }
                    if (TX.LockTime > unixTimestamp + MAXLOCKTIME)
                    {
                        txs.Add(TX);
                    }
                ...
                WinBlockPath = MineBlock(finalTX, prev, pKey, mUTXOP, WinBlockPath);
                if ( WinBlockPath.Length == 0) { return "";  }
                prev = GetBlockAtIndexInFile(prev.Index + 1,WinBlockPath);
                if (prev == null) { Print("err here"); return ""; }
            }
            return WinBlockPath;
        }
        public static string MineBlock(List<Tx> TXS, Block prevBlock, byte[] pKey, uint mUTXOP, string WinBlockPath)
        {
